...
# Ensure we can import from the 'server' package when this script is run directly
if __name__ == "__main__":
    sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), "..")))

from server.config import ConfigManager
from server.cluster import ClusterManager, RunnerManager
from server.federation import FederationManager
...
def _get_writable_log_dir() -> str:
    ...
    It first checks the 'GREENMESH_LOG_DIR' environment variable,
    ...
    """
    preferred_dir = os.getenv("GREENMESH_LOG_DIR", os.getenv("FLEXLLAMA_LOG_DIR", "logs"))
    ...
    fallback_dir = os.path.join(temp_base, f"greenmesh_logs_{user_id}")
    print(
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="GreenMesh")
    ...
    try:
        ...
        logger.info("Creating federation manager")
        federation_manager = FederationManager(config_manager, runner_manager)

        # Create API server
        logger.info("Creating API server")
        api_server = APIServer(config_manager, runner_manager, federation_manager)

        # Attach gpu_metrics_collector to runner_manager so federation can read GPU metrics if needed
        runner_manager.gpu_metrics_collector = api_server.gpu_metrics_collector
        ...
        # Start federation manager if enabled
