...
import { GpuMetricsResponse, ThroughputMetricsResponse } from '../types';
...
interface SvgThroughputChartProps {
  genHistory?: (number | null | undefined)[];
  promptHistory?: (number | null | undefined)[];
}

// Helper Throughput Chart Component
function SvgThroughputChart({ genHistory, promptHistory }: SvgThroughputChartProps) {
  ...
  const allVals = [...hGen, ...hPrompt].filter((v): v is number => v !== null && v !== undefined);
  ...
  const getCoordinates = (history: (number | null | undefined)[]) => {
...
interface MetricsCardsProps {
  ...
  throughputMetrics?: ThroughputMetricsResponse | null;
}

export default function MetricsCards({ gpuMetrics, throughputMetrics }: MetricsCardsProps) {
  // Build a map of GPU IDs to Cluster Names
  const buildGpuClusterMap = (associations?: GpuMetricsResponse['cluster_associations'] | GpuMetricsResponse['runner_associations']) => {
    const map: Record<string, string[]> = {};
    if (!associations) return map;
    Object.entries(associations).forEach(([clusterName, info]) => {
      let gpuKeys: string[] = [];
      if (Array.isArray(info.gpu_ids)) {
        gpuKeys = info.gpu_ids.map(String);
      ...
      gpuKeys.forEach((gpuKey) => {
        ...
        map[gpuKey].push(clusterName);
      });
    });
    return map;
  };

  const gpuClusterMap = buildGpuClusterMap(gpuMetrics?.cluster_associations || gpuMetrics?.runner_associations);
    ...
    return (
      <div className="gpu-metrics-grid">
        {gpuMetrics.gpus.map((gpu) => {
          ...
          const runners = gpuClusterMap[gpuKey] || gpuClusterMap[String(idx)] || [];

          const memUsed = gpu.memory_used_mb !== null && gpu.memory_used_mb !== undefined ? formatMb(gpu.memory_used_mb) : '--';
          const memTotal = gpu.memory_total_mb !== null && gpu.memory_total_mb !== undefined ? formatMb(gpu.memory_total_mb) : '--';
          const memPercent =
          ...
          const util = gpu.utilization_gpu_percent !== null && gpu.utilization_gpu_percent !== undefined ? `${Math.round(gpu.utilization_gpu_percent)}%` : '--';
          const temp = gpu.temperature_c !== null && gpu.temperature_c !== undefined ? `${Math.round(gpu.temperature_c)}°C` : '--';
          ...
          const isTempWarning = gpu.temperature_c !== null && gpu.temperature_c !== undefined && gpu.temperature_c > 85;
              ...
              <div className="throughput-card-header">
                ...
                <div className="throughput-stats">
                  <span className="throughput-chip">
                    Avg 1m: <strong>{model.avg_gen_tps_1m !== null && model.avg_gen_tps_1m !== undefined ? `${Number(model.avg_gen_tps_1m).toFixed(1)} t/s` : '--'}</strong>
                  </span>
                  <span className="throughput-chip">
                    Peak: <strong>{model.peak_gen_tps !== null && model.peak_gen_tps !== undefined ? `${Number(model.peak_gen_tps).toFixed(1)} t/s` : '--'}</strong>
                  </span>
                </div>
              </div>
